...
-- list (nice, we have co-recursive functions) 
...
-- with this parser, we need not even keep track of the parentheses, because
-- the absence of a parenthesis in the correct position will result in the parser
-- evaluating to Nothing

-- testing parseSExpr
input1 = "5"
input2 = "(bar (foo) 3 5 874)"
input3 = "( lots of ( spaces in ) this ( one ) )"
input4 = "(4())foo bar"  --should return Nothing
input5 = "(lambda x (lambda y ( plus x y   )))"

-- runParser parseSExpr <input> 
